# -*- coding: utf-8 -*-
"""
Created on Mon Aug  7 12:14:55 2023
...
for i in range(len(sentence)):
    crps = re.sub('[^a-zA-Z]',' ',sentence[i])
    crps = crps.lower()
    crps = crps.split()
    crps = [wordNet.lemmatize(word) for word in crps if word not in set(stopwords.words('english'))]
    crps = ' '.join(crps)
    corpus.append(crps)
    
    
#Implemet TF-IDF Model

from sklearn.feature_extraction.text import TfidfVectorizer

tf_idf = TfidfVectorizer()

Tf_IDF = tf_idf.fit_transform(corpus).toarray()
