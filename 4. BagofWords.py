# -*- coding: utf-8 -*-
"""
Created on Sun Aug  6 16:53:25 2023
...
for i in range(len(sentence)):
    ...
    crps = crps.lower()
    crps = crps.split()
    crps = [lem.lemmatize(word) for word in crps if word not in set(stopwords.words('english'))]
    crps = ' '.join(crps)
    corpus.append(crps)
...
Feature_Extractions = cv.fit_transform(corpus).toarray()
